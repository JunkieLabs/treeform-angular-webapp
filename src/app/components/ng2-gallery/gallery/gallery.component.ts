import {
    ...
    Input, OnChanges, OnDestroy, OnInit, Output, QueryList, SimpleChanges, ViewChild, ViewChildren, NgZone, Inject
} from '@angular/core'
...
export class GalleryComponent implements OnInit, OnDestroy, OnChanges {
    ...
    @Input()
    set galleryScrollReferenceTarget(target: HTMLElement) {
    ...
    @Input()
    set galleryScrollReferenceLink(targetLink: string) {
        //const link = Array.isArray(data) ? data[0] : data.split(" ")[0];
        //this.referenceLink = link;

        let target = this.document.getElementById(targetLink)
        if (target !== this.mRefTarget) {
            this.mRefTarget = target ? target : this._platform.isBrowser ? window : undefined;
            this._initScrollHandler();
        }
    ...
    constructor(public imageService: ImageService, public http: HttpClient,
    ...
    ngOnChanges(changes: SimpleChanges): void {
        ...
        if (changes['providedGalleryName'] != undefined) {
            ...
            // console.log("gallery name: ", changes);
            this.fetchDataAndRender()
        } else {
            this.render()
        }
        ...
        this._scrollTargetSubscription = this._ngZone.runOutsideAngular(() =>
            fromEvent<Event>(this.mRefTarget || window, 'scroll')
                .subscribe((event) => this._ngZone.run(() => {
                    this.doSomethingOnScroll(event)
                })));
    }
